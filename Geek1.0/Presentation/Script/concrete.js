    ...
    xhr.onreadystatechange=function(){
        if (xhr.readyState==4) {
            if (xhr.status>=200&&xhr.status<300||xhr.status==304) {
                ...
                {
                    if((json.data)[i].id==purpose)
                    {
                        var lunbo = (json.data)[i].photo;
                        //拼接轮播图
                        for(var j=1;j<lunbo.length;j++)
                            {
                                str1 += "<img src='image/"+(json.data)[i].photo[j]+"' class='bigimg'>";
                            }
                        //拼接它的介绍内容
                        str2 = "<h1>"+(json.data)[i].name+"</h1><p>"+(json.data)[i].ins+"</p>";
